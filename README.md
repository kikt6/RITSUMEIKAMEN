...
ページ上では、今日から今後2週間分だけ表示します。表示日数を変える場合は、`content.js` の `coop.displayDays` を変更します。
...
このページはログインなしの静的HTMLです。URLを知っている人は閲覧できます。本当に閲覧者を制限したい場合は、サーバー側の認証や Firebase / Supabase などのログイン機能が必要です。
